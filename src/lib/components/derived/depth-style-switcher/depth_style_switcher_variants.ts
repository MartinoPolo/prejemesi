...
export const depthStyleSwitcherVariants = tv({
	slots: {
		...
		choice: 'relative min-h-12 min-w-0 justify-center rounded-btn border-2 border-border-strong bg-card px-1.5 text-center text-(length:--text-xs) font-bold text-foreground whitespace-nowrap shadow-sticker hover:shadow-sticker-lift active:shadow-sticker data-[state=on]:border-border-strong data-[state=on]:bg-[var(--selection-tint)] data-[state=on]:text-foreground',
		indicator:
