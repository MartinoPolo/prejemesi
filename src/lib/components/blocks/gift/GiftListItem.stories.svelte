...
<Story name="Unavailable comparison">
	{#snippet template()}
		<div class="flex w-full max-w-[40rem] flex-col gap-4">
			<div class="flex flex-col gap-2">
				<p class="text-sm font-bold">Dostupný</p>
				<GiftListItem gift={COMPARISON_AVAILABLE} role={WISHLIST_ROLES.visitor} />
			</div>
			<div class="flex flex-col gap-2">
				<p class="text-sm font-bold">Rezervovaný jiným dárcem</p>
				<GiftListItem gift={COMPARISON_RESERVED_BY_OTHERS} role={WISHLIST_ROLES.visitor} />
			</div>
		</div>
	{/snippet}
</Story>
