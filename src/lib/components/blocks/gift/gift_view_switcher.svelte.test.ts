...
describe('GiftViewSwitcher toggle selection (fixes: re-click deselects both items)', () => {
	...
	it('preserves semantic geometry while painting a flush connected backing and button face', async () => {
		...
		for (const [viewportWidth, expected] of [
			[390, { rootWidth: 79, size: 40, segment: 38, backingWidth: 79 }],
			[800, { rootWidth: 67, size: 32, segment: 32, backingWidth: 66 }],
		] as const) {
			...
			for (const item of [card, list]) {
				...
				expect(item.querySelector('svg')!.getBoundingClientRect().width).toBe(16);
				expect(item.querySelector('svg')!.getBoundingClientRect().height).toBe(16);
			}

			for (const selected of [card, list]) {
				if (selected.getAttribute('data-state') !== 'on') {
					await selected.click();
				}
				const selectedSurface = selected.querySelector('.elevation-surface')!;
				...
				if (viewportWidth >= 640) {
					expect(parseFloat(backingStyle.left)).toBe(selected === card ? 1 : 0);
					expect(parseFloat(backingStyle.right)).toBe(selected === list ? 1 : 0);
				}
			}
		}
		await screen.unmount();
	...
	it('uses one contextual tray shadow with no frame and only a selected Button face', async () => {
		...
		try {
			...
			const cardSurface = card.querySelector('.elevation-surface') as HTMLElement;
			const listSurface = list.querySelector('.elevation-surface') as HTMLElement;
			const backingStyle = getComputedStyle(group, '::before');
			...
			expect(getComputedStyle(group, '::before').boxShadow).toBe(backingStyle.boxShadow);
		} finally {
			await screen.unmount();
	...
	it('keeps the inactive connected surface paint static on hover', async () => {
		const rootStyle = document.documentElement.style;
		const properties = ['--accent', '--foreground', '--muted-foreground'] as const;
		const previousProperties = properties.map((property) => ({
			property,
			value: rootStyle.getPropertyValue(property),
			priority: rootStyle.getPropertyPriority(property),
		...
		rootStyle.setProperty('--accent', 'rgb(11, 22, 33)');
		rootStyle.setProperty('--foreground', 'rgb(44, 55, 66)');
		rootStyle.setProperty('--muted-foreground', 'rgb(77, 88, 99)');
		...
		try {
			const inactiveSurface = screen
				.getByTestId(`gift-view-${GIFT_VIEW_MODES.list}`)
				.element()
				.querySelector('.elevation-surface') as HTMLElement;
			const restingPaint = {
