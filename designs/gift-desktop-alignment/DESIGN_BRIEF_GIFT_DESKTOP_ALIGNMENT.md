...
- Desktop List: full-height image with a compact, bounded preferred width based on available inline space and a square minimum-height floor, never a width derived from row height. Ordinary rows prefer square frames; taller content or overlays may make individual frames portrait without equalizing row heights across gifts or groups. Keep one-line title and description and minimal content-driven row heights. Price below links may share a bottom row with actions when the measured available width comfortably permits; otherwise separate them. This sharing is not a change to the approved desktop Card price placement.
