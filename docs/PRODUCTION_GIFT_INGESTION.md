...
2. **Migrate before dependent code is deployed.** Apply the committed additive migrations with `pnpm db:migrate:prod` (direct Neon URL from the ignored `.env.production`; never Hyperdrive, `db:push`, or seed). All pending migrations `0012_add_new_gift_digests.sql` through `0016_ambiguous_whizzer.sql` must complete before approving deployment of code that reads them.
