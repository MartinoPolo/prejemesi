...
**Crop target** — A gift consumer surface group by aspect family: `card` (~2.78:1, retired legacy fallback), `detail` (~0.5, retired legacy fallback — the visitor detail modal now shows the uncropped photo instead, see below), `square` (4:3 gift-card-grid + list-thumbnail family, issue #183 — the name stays `square` for backward-compatible persistence despite the aspect no longer being square; the reservation-modal thumbnail also renders through it as a small approximate, non-WYSIWYG icon). Manual crops persist per target in `image_meta.targets`; targets without one keep automatic center cover-fit framing. Aspect specs live in `crop_targets.ts` (single source for editor stage, previews, and e2e asserts). The visitor gift detail view is NOT a crop-target consumer — it renders the full photo at its natural aspect ratio inside a height cap.
