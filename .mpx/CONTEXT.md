...
**Image editor mode** — The user-facing three-mode model in the gift/wishlist image editors (#116 follow-up): Fill („Vyplnit", automatic centered cover), Fit („Přizpůsobit", letterboxed), Manual („Ručně", user-drawn per-target/per-slot crops). Maps onto persisted fit modes via `editor_modes.ts`; Fill and Manual both persist `cover-crop`. Clicking a preview tile or wheel-zooming a plain preview promotes to Manual — the tiles are the only crop-target switcher (round 3 removed the per-target radio picker). Manual zoom ranges from the target's contain zoom (whole image, one-axis letterbox) up to 300 %. In the gift modal the mode toggle sits in the image column above the preview, with three same-height tiles at the column's lower edge: card, one merged `square` tile (list + reservation share the target), and a really narrow 1:2 detail tile that exists as a switcher (the big left preview IS the detail target).
...
**Crop target** — A gift consumer surface group by aspect family: `card` (~2.78:1 banner), `detail` (~0.5 modal column), `square` (1:1 list + reservation thumbs). Manual crops persist per target in `image_meta.targets`; targets without one keep automatic center cover-fit framing. Aspect specs live in `crop_targets.ts` (single source for editor stage, previews, and e2e asserts).
