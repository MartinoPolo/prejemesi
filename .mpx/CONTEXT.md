...
**Image editor mode** — The user-facing three-mode model in the gift/wishlist image editors (#116 follow-up): Fill („Vyplnit", automatic centered cover), Whole picture („Celý obrázek", letterboxed), Manual („Ručně", user-drawn per-target/per-slot crops). Maps onto persisted fit modes via `editor_modes.ts`; Fill and Manual both persist `cover-crop`. Clicking a preview tile or wheel-zooming a plain preview promotes to Manual. Manual zoom ranges from the target's contain zoom (whole image, one-axis letterbox) up to 300 %. Gift live previews are two floating tiles in the image column — card + one merged `square` tile (list + reservation share the target); the big left preview IS the detail target.
